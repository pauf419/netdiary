import { FC, useState } from "react";
import cl from "./FileInput.module.sass"

interface IProps {
    onChange: (url:string, data:any) => void
    defaultImg: string
}

const FileInput: FC<IProps> = ({onChange, defaultImg}) => {

    const [previewFile, setPreviewFile] = useState<File | null>(null)
    const [fileUrl, setFileUrl] = useState<string>("")

    const previewSelectedAvatar = (data:any) => {
        setPreviewFile(data)
        const url = URL.createObjectURL(data)
        setFileUrl(url)
        console.log(data)
        onChange(url, data)
    }

    return (
        <div className={cl.FileInputContainer}>
            <input type="file" id={"avatar"} style={{display: "none"}} onChange={(e: React.ChangeEvent<HTMLInputElement>) => e.target.files && previewSelectedAvatar(e.target.files[0])}/>
            <label htmlFor={"avatar"} className={cl.FileInputInner}>
                <div className={cl.Blurer}>
                    <span className={cl.Changer_ic}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                            <path d="M8 2a5.53 5.53 0 0 0-3.594 1.342c-.766.66-1.321 1.52-1.464 2.383C1.266 6.095 0 7.555 0 9.318 0 11.366 1.708 13 3.781 13h8.906C14.502 13 16 11.57 16 9.773c0-1.636-1.242-2.969-2.834-3.194C12.923 3.999 10.69 2 8 2zm2.354 5.146a.5.5 0 0 1-.708.708L8.5 6.707V10.5a.5.5 0 0 1-1 0V6.707L6.354 7.854a.5.5 0 1 1-.708-.708l2-2a.5.5 0 0 1 .708 0l2 2z"/>
                        </svg>
                    </span>
                </div>
                <img className={cl.Img} src={fileUrl ? fileUrl : defaultImg}/>
            </label>
        </div>
    )
}

export default FileInput